...
# curl_cffi замість requests — імітує TLS/JA3-відбиток справжнього Chrome,
# а не тільки заголовки. Це обходить блокування, побудоване на аналізі
# TLS-рукостискання (типово для Cloudflare), на відміну від urllib3/requests,
# чий відбиток видає "це не браузер" незалежно від User-Agent.
...
# Проксі (опційно) — для GitHub Actions, якщо навіть TLS-імперсонація
# не рятує через блок IP-діапазону. Формат: http://user:pass@host:port
...
# Базовий інтервал між запитами — окремий для кожної фази.
PHASE_INTERVALS = {
    "catalog": 0.45,   # ≈ 2.2 запити/сек
    "details": 0.15,   # трохи обережніше, ніж було: TLS-імперсонація нова,
                        # спершу варто пересвідчитись, що вона не ловить 403/429
}
JITTER_MAX = 0.25
...
def get_session() -> requests.Session:
    if not hasattr(thread_local, "session"):
        s = requests.Session(impersonate="chrome", proxies=PROXIES)
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                if retry_after and str(retry_after).strip().isdigit() and int(retry_after) > 0:
...
def load_category(category_name: str, url_base: str) -> List[Dict]:
    ...
    while page <= MAX_PAGES_PER_CAT:
        ...
        # просто повертають останню сторінку знову.
...
def main() -> pd.DataFrame:
    ...
    if df.empty:
        print("❌ Каталог порожній — перевір з'єднання, TLS-імперсонацію або селектори.")
        return df
    ...
    if STATS["latency_count"]:
        ...
        print(f"  Середній час відповіді сервера: {avg_latency:.2f} сек. "
